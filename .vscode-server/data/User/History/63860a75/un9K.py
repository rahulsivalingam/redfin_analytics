...
def extract_data(**kwargs):
    ...
    for file in glob.glob("/home/ubuntu/data_files/*.csv"):
        os.remove(file)

    objects = s3.list_objects_v2(Bucket=raw_data_bucket, Prefix=prefix)
    if "Contents" in objects:
        for obj in objects["Content"]:
            s3.delete_object(Bucket = raw_data_bucket, Key = obj["Key"])
